<script lang="ts">
  ...
  import type { Network, ChannelScore } from '$lib/types';
  ...
  function scoreChannel(channel: number, networks: Network[]): ChannelScore {
    ...
    for (const n of networks) {
      // 2.4G channels overlap
      const overlaps = selectedBand === '2.4'
        ? Math.abs(n.channel - channel) <= 4
        : n.channel === channel;
  ...
  const recommendations = $derived(
    channelScores()
      .filter(c => c.recommendation === 'best' || c.recommendation === 'good')
      .slice(0, 3)
  );

  function scoreColor(score: ChannelScore): string {
    return score.recommendation === 'best' ? '#22c55e' :
           score.recommendation === 'good' ? '#84cc16' :
           score.recommendation === 'fair' ? '#eab308' : '#ef4444';
  }
</script>
...
<div class="p-4">
  ...
  <div class="flex gap-2 mb-4">
    <button
      class={cn(
        ...
        selectedBand === '2.4' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200'
      )}
    ...
    <button
      class={cn(
        'px-4 py-2 rounded transition-colors',
        selectedBand === '5' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200'
      )}
  ...
  <div class="relative h-32 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
    {#each channelScores() as cs, i}
      ...
      <div
        class="absolute bottom-6 rounded-t transition-all"
        style="left: {x}%; width: {w}%; height: {cs.score}%; background-color: {scoreColor(cs)}"
        title="CH{cs.channel}: {cs.score}分"
      ></div>
    {/each}
  ...
  <div class="mt-4 space-y-1 max-h-48 overflow-y-auto">
    {#each channelScores().filter(c => c.networks.length > 0) as cs}
      <div class="flex items-center justify-between text-sm p-2 bg-gray-50 dark:bg-gray-800 rounded border border-gray-100 dark:border-gray-700">
        ...
        <span class="text-gray-500 dark:text-gray-400 truncate mx-2">
          {cs.networks.join(', ') || '无'}
        </span>
        <span class={cn(
          'font-bold',
          cs.recommendation === 'best' && 'text-green-600',
          cs.recommendation === 'good' && 'text-lime-600',
          cs.recommendation === 'fair' && 'text-yellow-600',
          cs.recommendation === 'avoid' && 'text-red-600'
        )}>
          {cs.score}
        </span>
      </div>
