...
test.describe('UniFi WiFi Scanner UI', () => {
  ...
  test('should display header with title and buttons', async ({ page }) => {
    // Check title
    await expect(page.locator('h1:has-text("UniFi")')).toBeVisible();
  ...
  test('should switch between tabs', async ({ page }) => {
    // Start on networks tab
    await expect(page.getByRole('button', { name: '网络列表' })).toHaveClass(/bg-blue-500/);
    
    // Switch to channels tab
    await page.getByRole('button', { name: '信道分析' }).click();
    await expect(page.getByRole('button', { name: '信道分析' })).toHaveClass(/bg-blue-500/);
    
    // Switch to groups tab
    await page.getByRole('button', { name: '网络分组' }).click();
    await expect(page.getByRole('button', { name: '网络分组' })).toHaveClass(/bg-blue-500/);
  });

  test('should filter by band', async ({ page }) => {
    await page.waitForTimeout(3000);

    // Check band filter buttons - use regex for more flexibility
    const allBtn = page.getByRole('button', { name: '全部' });
    // The band buttons show as "2.4G", "5G", "6G" - use regex to match
    ...
    await expect(band5Btn).toHaveClass(/bg-blue-500/);
  });
  ...
  test('should display network count', async ({ page }) => {
    ...
    const countText = page.locator('text=/共 \\d+ 个网络/');
  ...
  test('should show scan stats if available', async ({ page }) => {
    ...
    const scanDuration = page.locator('text=/扫描耗时/');
